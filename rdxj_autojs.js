...
function doTask() {
    ...
    while (hasTask) {
        if (textMatches(/.*[0-9]秒.*/).exists() && textMatches(/.*[0-9]秒.*/).findOnce().parent().child(8).text() == "去完成") {
            ...
            sleep(10000);
        } else if (textContains("浏览").exists() && !textContains("浏览加购").exists() && textContains("浏览").findOnce().parent().child(8).text() == "去完成") {
            ...
            sleep(3000);
            toast("浏览完成，返回");
        } else if (textStartsWith("逛逛").exists() && textStartsWith("逛逛").findOnce().parent().child(8).text() == "去完成") {
            textStartsWith("逛逛").findOnce().parent().child(8).click();
            sleep(3000);
            toast("浏览完成，返回");
        } else if (textStartsWith("去逛").exists() && textStartsWith("去逛").findOnce().parent().child(8).text() == "去完成") {
            textStartsWith("去逛").findOnce().parent().child(8).click();
            sleep(3000);
            toast("浏览完成，返回");

...
let shareCodeMap = new Map();//助力码成功邀请好友次数 MAP
let shareCodes = []
let option = {};
!(async () => {
  await requireConfig();
...
async function receiveFriendInvite() {
  ...
  for (let i = 0; i < newShareCodes.length; i++) {
    ...
    if (code === $.farmInfo.farmUserPro.shareCode) {
      console.log('自己不能邀请自己\n')
      continue
    }
    //console.log(`开始助力: ${code}`);
    let addFriendCount = shareCodeMap.get(code);
    await inviteFriend(code);
    if ($.inviteFriendRes.helpResult.code === '0') {
      console.log(`【接受好友邀请-成功】：${$.inviteFriendRes.helpResult.masterUserInfo.nickName}：${code}`)
      shareCodeMap.set(code, addFriendCount + 1);
      if ((addFriendCount + 1) >= 4) {
        //插入头部
        fullHelpIndexArr.unshift(i);
        console.log(`${$.inviteFriendRes.helpResult.masterUserInfo.nickName}：${code} 已满助力`);
      }
    } else if ($.inviteFriendRes && $.inviteFriendRes.helpResult && $.inviteFriendRes.helpResult.code === '17') {
      console.log(`【接受好友邀请-失败】：${code} 已是好友`)
    }
  }
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if (jdFruitShareArr[$.index - 1]) {
      ...
      for (let i = 0; i < newShareCodes.length; i++) {
        shareCodeMap.set(newShareCodes[i], 0);
      }
    }/* else {
